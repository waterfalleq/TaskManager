...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("user2@example.com", "Another1@Pwd")
    ]
)
...
def test_register_user_duplicate_email(client: TestClient):
    # Create first user
    client.post("/auth/register", json={"email": "dup@example.com", "password": "Strong1!"})
    # Try to register again with same email
    response = client.post("/auth/register", json={"email": "dup@example.com", "password": "Strong1!"})
    assert response.status_code == 400 or response.status_code == 409  # depends on your CRUD
...
def test_register_user_invalid_password(client: TestClient, password):
    response = client.post("/auth/register", json={"email": "testpass@example.com", "password": password})
